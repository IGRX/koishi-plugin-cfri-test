...
export function get_info_status(ctx: Context) {
  ...
  .action(async ({session}) => {
    try {
      return session.send('用户ID: ' + session.event.user.id)
    } catch (error) {
      return session.send('获取用户ID失败，请联系频道主')
    }
  })
  ...
  .action(async ({session}) => {
    try {
      return session.send('平台名: ' + session.event.platform + '\n'
      ...
      return session.send('获取状态信息失败，请联系频道主')
    }
  })
  try {
